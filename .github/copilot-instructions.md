...
- **DB**: Async Postgres via `app/db/session.py` using `get_db` (async context manager)
...
- **Always use `get_db`**: Open sessions via the async context manager; do not introduce alternative patterns.
...
- Provide class/instance methods. You may open sessions inside model methods using `get_db`.
